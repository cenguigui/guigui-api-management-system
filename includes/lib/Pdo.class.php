...
class PdoHelper
{
	private $sqlPrefix = "api_";//SQL数据表前缀识别字符
	private $db;
	...
	function __construct($dbconfig)
	{
		...
		try {
			...
			$options = array(
				...
				\PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8mb4" // 直接设置字符集
			);
			// PHP 5.3.6+ 支持持久连接，提升性能
			if (version_compare(PHP_VERSION, '5.3.6', '>=')) {
				$options[\PDO::ATTR_PERSISTENT] = true;
			}
			// PHP 5.3.7+ 支持 ATTR_EMULATE_PREPARES，但某些 MySQL 版本可能不支持，根据版本动态设置
			if (version_compare(PHP_VERSION, '5.3.7', '>=')) {
				// 在 PHP 8.0+ 中，某些 MySQL 驱动可能需要启用模拟预处理
				if (version_compare(PHP_VERSION, '8.0.0', '>=')) {
					// PHP 8.0+ 默认行为更严格，根据实际情况决定
					$options[\PDO::ATTR_EMULATE_PREPARES] = false;
				} else {
					$options[\PDO::ATTR_EMULATE_PREPARES] = false;
				}
			}
			$this->db = new \PDO($dsn, $dbconfig['user'], $dbconfig['pwd'], $options);
		} catch (\Exception $e) {
			exit('链接数据库失败:' . $e->getMessage());
		} catch (\Throwable $e) {
			// PHP 7.0+ 支持 Throwable，捕获所有错误
			exit('链接数据库失败:' . $e->getMessage());
		}
	...
	private function _select($table, $fields = '*', $where = array(), $sort = null, $limit = null){
		...
		$sql = ' FROM api_'.$table.$conditions["_where"];
		if(is_array($limit)){
	...
	public function insert($table, $data){
		...
		$rowCount = $this->exec("INSERT INTO api_".$table." (".implode(', ', $keys).") VALUES (".implode(', ', $marks).")", $values);
		if($rowCount){
	...
	public function update($table, $data, $where){
		...
		$conditions = $this->_where($where);
		$rowCount = $this->exec("UPDATE api_".$table." SET ".$update.$conditions["_where"], $conditions["_bindParams"] + $values);
		return $rowCount;
	}
	...
	public function delete($table, $where){
		$conditions = $this->_where($where);
		$rowCount = $this->exec("DELETE FROM api_".$table.$conditions["_where"], $conditions["_bindParams"]);
		return $rowCount;
	}
	...
	public function count($table, $where){
		$conditions = $this->_where($where);
		$count = $this->getColumn("SELECT COUNT(*) FROM api_".$table.$conditions["_where"], $conditions["_bindParams"]);
		return $count;
	}
